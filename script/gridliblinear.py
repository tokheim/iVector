...
if not is_win32:
       svmtrain_exe = "../liblinear-1.8/train"
...
fold = 5
c_begin, c_end, c_step = -8, 2, 1
g_begin, g_end, g_step =  0, 0, 1
...
class SSHWorker(Worker):
    def __init__(self,name,job_queue,result_queue,host):
        ...
        self.cwd = os.getcwd()
    def run_one(self,c,g):
        cmdline = 'ssh -x {0} "cd {1}; {2} -c {3} -v {5} {6} {7}"'.format \
          (self.host,self.cwd, \
           svmtrain_exe,c,fold,pass_through_string,dataset_pathname)
        result = Popen(cmdline,shell=True,stdout=PIPE).stdout
        for line in result.readlines():
